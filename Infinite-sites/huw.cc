...
double step(infSites &infsites, const Matrix &prop, gsl_rng *gen,
            const double theta) {
  int ind = infsites.sample_row(gen);
  double ret = 0;
  if (infsites.row_counts[ind] > 1) {
    ret =
        ((infsites.row_counts[ind] - 1) / (infsites.sample_size - 1 + theta)) /
        (infsites.row_prop[ind] / infsites.total_prob);
    ...
    infsites.recompute_proposal(prop);
  } else {
    int j = infsites.sample_singleton(ind, gen);
    // need to store proposal here because remove_mutation can change it.
    double denom = infsites.row_prop[ind] / infsites.total_prob;
    j = infsites.remove_mutation(ind, j, prop);
    ret = (double(infsites.row_counts[j]) / infsites.sample_size) *
          (theta / (infsites.sample_size - 1 + theta)) / denom;
  }
  return log(ret);
}

void smc(const infSites &infsites, const Matrix &prop, const double theta,
    ...
    while (boundary > target) {
      ...
      for (int i = 0; i < particle_no; i++) {
        while (temp_samples[i].sample_size > boundary) {
          likelihoods[i] += step(temp_samples[i], prop, gen, theta);
        }
      }
      if (output == 1) {
...
int main(int argc, char **argv) {
  ...
  int col_max = 0;
  for (int i = 0; i < infsites.data.ncol; i++) {
    col_max = std::max(col_max, infsites.col_counts[i]);
  }

  Matrix prop("proposal-" + std::to_string(infsites.sample_size) + ".txt");
  infsites.recompute_proposal(prop);
  ...
  smc(infsites, prop, theta, min_particles, max_particles, output,
      resampling_frac, gen);
