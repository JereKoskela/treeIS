...
double step(infSites &infsites, gsl_rng *gen, const double theta) {
  ...
  int k = 0;
  int r = 0;
  std::vector<double> proposal(infsites.data.nrow, 0);
  for (int i = 0; i < infsites.data.nrow; i++) {
    if (infsites.row_counts[i] > 1) {
      nc += infsites.row_counts[i] - 1;
      proposal[i] = infsites.row_counts[i] - 1;
    } else {
      k = infsites.singleton_count(i);
      if (k > 1) {
        nc += theta / infsites.sample_size;
        proposal[i] = theta / infsites.sample_size;
      } else if (k == 1) {
        k = infsites.sample_singleton(i, gen);
        r = infsites.match(i, k);
        if (r < infsites.data.nrow) {
          nc +=
              theta * double(infsites.row_counts[r] + 1) / infsites.sample_size;
          proposal[i] =
              theta * double(infsites.row_counts[r] + 1) / infsites.sample_size;
        } else {
          nc += theta / infsites.sample_size;
  ...
  if (infsites.row_counts[ind] > 1) {
    ...
    ret = (double(infsites.row_counts[j]) / infsites.sample_size) *
          (theta / (infsites.sample_size - 1 + theta)) / (proposal[ind] / nc);
  }
  return log(ret);
}
